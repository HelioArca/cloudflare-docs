...
When Cloudflare caches a response, `Set-Cookie` headers are always removed from the cached copy. There is currently no way to cache a response while preserving origin `Set-Cookie` headers. If your origin needs to set cookies, the response must either bypass cache or be served as a cache MISS.

The RFC-compliant `cache-control: no-cache="set-cookie"` directive (cache the body but always revalidate for fresh headers) is not currently supported.
