...
Cloudflare supports [ML-DSA](https://csrc.nist.gov/pubs/fips/204/final) post-quantum signatures in two origin-facing features:
...
The following commands create a private certificate authority and a leaf certificate that chains to it. Repeat once for an AOP client certificate, and once for a COTS server-facing certificate if you manage that side too. The example uses ML-DSA-44 — substitute `mldsa65` or `mldsa87` for the `-algorithm` value to use a higher-security parameter set.
...
The `-provparam ml-dsa.output_formats=seed-only` flag is required so that the private key is written as the 32-byte FIPS 204 seed (approximately 128 bytes of PEM) rather than the expanded private key. This is the only form Cloudflare currently accepts on upload.
...
The output should show `Peer signature type: mldsa44` and `Negotiated TLS1.3 group: X25519MLKEM768`.
